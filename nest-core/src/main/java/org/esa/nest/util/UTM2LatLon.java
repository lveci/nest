...
public class UTM2LatLon
{
    ...
    private double calcLatitude(final double northing, final double easting)
    {
        ...
        final double ei = (1 - Math.pow((1 - e * e), (1 / 2.0)))
                / (1 + Math.pow((1 - e * e), (1 / 2.0)));

        final double ca = 3 * ei / 2 - 27 * Math.pow(ei, 3) / 32.0;

        final double cb = 21 * Math.pow(ei, 2) / 16.0 - 55.0 * Math.pow(ei, 4) / 32.0;
        final double cc = 151 * Math.pow(ei, 3) / 96.0;
        final double cd = 1097 * Math.pow(ei, 4) / 512.0;
        final double phi1 = mu + ca * Math.sin(2 * mu) + cb * Math.sin(4 * mu) + cc * Math.sin(6 * mu) + cd
                * Math.sin(8 * mu);

        final double n0 = a / Math.pow((1 - Math.pow((e * Math.sin(phi1)), 2)), (1 / 2.0));

        final double r0 = a * (1 - e * e) / Math.pow((1 - Math.pow((e * Math.sin(phi1)), 2)), (3 / 2.0));
        final double fact1 = n0 * Math.tan(phi1) / r0;

        final double _a1 = 500000 - easting;
        final double dd0 = _a1 / (n0 * k0);
        final double fact2 = dd0 * dd0 / 2.0;

        final double t0 = Math.pow(Math.tan(phi1), 2);
        final double Q0 = e1sq * Math.pow(Math.cos(phi1), 2);
        final double fact3 = (5 + 3 * t0 + 10 * Q0 - 4 * Q0 * Q0 - 9 * e1sq) * Math.pow(dd0, 4) / 24.0;

        final double fact4 = (61 + 90 * t0 + 298 * Q0 + 45 * t0 * t0 - 252 * e1sq - 3 * Q0
                * Q0)
                * Math.pow(dd0, 6) / 720.0;

        final double lof1 = _a1 / (n0 * k0);
        final double lof2 = (1 + 2 * t0 + Q0) * Math.pow(dd0, 3) / 6.0;
        final double lof3 = (5 - 2 * Q0 + 28 * t0 - 3 * Math.pow(Q0, 2) + 8 * e1sq + 24 * Math.pow(t0, 2))
                * Math.pow(dd0, 5) / 120.0;
        final double _a2 = (lof1 - lof2 + lof3) / Math.cos(phi1);
        _a3 = _a2 * 180 / Math.PI;
