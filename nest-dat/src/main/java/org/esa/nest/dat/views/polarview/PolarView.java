...
public class PolarView extends JPanel {
    ...
    private static final Color waveColorTable[] = (new Color[] {
            Color.white, new Color(255, 210, 255), new Color(170, 170, 255), new Color(125, 210, 255),
    ...
    private void createPlot(RasterDataNode rasterNode, int rec) {
        ...
        data.setColorScale(ColorScale.newCustomScale(waveColorTable, cRange));
        graphView.setData(data);
    }


    public float[][] getSpectrum(boolean getReal) {
    ...
    private static float[][] resampleSpectrum(float spectrum[][], int resampling)
    {
        ...
        for(int j = 0; j < nWl; j++)
            resampledSpectrum[nTh * 2 - 1][j] = spectrum[nTh - 1][j];
