...
public class ProductGrabberConfig {
    ...
    private static final String RECURSION_DIR = "_RecursionBaseDir";
    ...
    public void setRepositories(final Repository[] repositories) {
        ...
        for (int i = 0; i < repositories.length; i++) {
            ...
            _properties.setPropertyString(propertyName+RECURSION_DIR, repository.getRecursionBaseDir().getPath());
        }
    }
    ...
    public Repository[] getRepositories() {
        ...
        for (int i = 0; i < _properties.getProperties().size(); i++) {
            ...
            final String recursionBaseDirPath = _properties.getPropertyString(propertyName+RECURSION_DIR);
            if (baseDirPath != null) {
                final File baseDir = new File(baseDirPath);
                final File recursionBaseDir = new File(recursionBaseDirPath);
                if (baseDir.exists()) {
                    repositoryList.add(new Repository(baseDir, recursionBaseDir));
                }
            }
        }
    ...
    private void clearRepositoryList() {
        for (int i = 0; i < _properties.getProperties().size(); i++) {
            ...
            if (_properties.getPropertyString(propertyName) != "") {
                ...
                _properties.setPropertyString(propertyName+RECURSION_DIR, null);
            }
        }
    }
