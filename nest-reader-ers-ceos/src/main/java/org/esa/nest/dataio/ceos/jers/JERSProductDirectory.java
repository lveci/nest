...
class JERSProductDirectory {
    ...
    private void addGeoCoding(final Product product) throws IllegalCeosFormatException,
      ...
      /*  //final String usedProjection = _leaderFile.getUsedProjection();
        ...
        if (JERSConstants.MAP_PROJECTION_RAW.equalsIgnoreCase(usedProjection)) {
            final Band[] bands = product.getBands();
            for (int i = 0; i < bands.length; i++) {
                final Band band = bands[i];
                final JERSImageFile imageFile = getImageFile(band);
        ...
        } else if (JERSConstants.MAP_PROJECTION_UTM.equalsIgnoreCase(usedProjection)) {
        ...
        } else if (JERSConstants.MAP_PROJECTION_PS.equalsIgnoreCase(usedProjection)) {
