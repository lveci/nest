...
//    }
    ...
    private void estimateFlatEarthPolynomial() {
        ...
        // this is where the reference phase polynomial is computed
        int polyDegree = 5; // for flat earth phase
        int numberOfCoefficients = numberOfCoefficients(polyDegree);

        double[][] position = distributePoints(numberOfPoints, rectangle);

        // setup observation and design matrix
        DoubleMatrix y = new DoubleMatrix(numberOfPoints);
        DoubleMatrix A = new DoubleMatrix(numberOfPoints, numberOfCoefficients);
